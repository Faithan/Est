...
// Fetch reservation details if manage_id is set
if (isset($_GET['manage_id'])) {
    $manage_id = $_GET['manage_id'];
    $manage_query = "SELECT * FROM reserve_room_tbl WHERE reserve_id = $manage_id";
    $manage_data = mysqli_fetch_assoc(mysqli_query($con, $manage_query));
}
    ...
    function renderTable($con, $status)
    {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            echo '<tr>
                ...
                <td><div class="edit-btn"><a href="' . $status . '.php?manage_id=' . $row['reserve_id'] . '"><i class="fa-solid fa-arrow-up-right-from-square"></i>Open</a></div></td>
            </tr>';
        }
